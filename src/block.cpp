...
Block::Block(int index, const std::string &previousHash,
    ...
    : index(index), previousHash(Crypto::normaliseHash(previousHash)),
      transactions(transactions), minerAddress(minerAddress),
      difficulty(difficulty), timestamp(timestamp), nonce(nonce), reward(0.0) {
  hash = "";
...
std::vector<unsigned char> Block::getSignatureMessage() const {
  ...
  if (hashed.size() != 32) {
    std::cerr << "❌ [getSignatureMessage] Error: hash length is "
              << hashed.size() << " instead of 32 bytes!\n";
  }
...
bool Block::mineBlock(int difficulty) {
  ...
  if (proofStr.rfind("error-proof:", 0) == 0 || proofStr.size() < 200) {
    std::cerr << "❌ [mineBlock] zk-STARK proof invalid (" << proofStr.size()
              << " bytes). Aborting mining.\n";
    return false;
  }

  zkProof.assign(proofStr.begin(), proofStr.end());
#ifdef LOG_DEBUG
  std::cout << "✅ zk-STARK Proof Generated. Size: " << zkProof.size()
            << " bytes\n";
#endif
  ...
  if (dilithiumSignature.empty() || publicKeyDilithium.empty()) {
    std::cerr
        << "❌ [mineBlock] Dilithium signature/public key missing. Abort.\n";
    return false;
  }
  ...
  std::cerr << "[mineBlock] DONE for idx=" << getIndex()
            << ", hash=" << getHash() << ", prev=" << getPreviousHash()
...
void Block::signBlock(const std::string &minerAddress) {
  ...
  if (msgBytes.size() != 32) {
    std::cerr
        << "❌ [ERROR] Message hash must be 32 bytes! Aborting signBlock.\n";
    return;
  }

  // Dilithium
  if (!Crypto::fileExists(DBPaths::getKeyDir() + minerAddress +
                          "_dilithium.key")) {
#ifdef LOG_DEBUG
...
//
double Block::getReward() const { return reward; }

void Block::setReward(double r) { reward = r; }
...
Json::Value Block::toJSON() const {
  ...
  block["dilithiumSignature"] = Crypto::base64Encode(
      std::string(dilithiumSignature.begin(), dilithiumSignature.end()));
  block["falconSignature"] = Crypto::base64Encode(
      std::string(falconSignature.begin(), falconSignature.end()));
  block["zkProof"] =
      Crypto::base64Encode(std::string(zkProof.begin(), zkProof.end()));
...
Block Block::fromJSON(const Json::Value &blockJson) {
  ...
  Block block(blockJson["index"].asInt(), blockJson["previousHash"].asString(),
              txs, blockJson["minerAddress"].asString(),
              blockJson["difficulty"].asInt(),
              blockJson["timestamp"].asUInt64(), blockJson["nonce"].asUInt64());
  ...
  // ✅ Decode base64-encoded binary fields
  {
    std::string decoded = Crypto::base64Decode(
        blockJson.get("dilithiumSignature", "").asString());
    block.dilithiumSignature =
        std::vector<unsigned char>(decoded.begin(), decoded.end());
  }

  {
    std::string decoded =
        Crypto::base64Decode(blockJson.get("falconSignature", "").asString());
    block.falconSignature =
        std::vector<unsigned char>(decoded.begin(), decoded.end());
  }

  std::string zkStr =
      Crypto::base64Decode(blockJson.get("zkProof", "").asString());
  block.zkProof = std::vector<uint8_t>(zkStr.begin(), zkStr.end());

  {
    std::string decoded = Crypto::base64Decode(
        blockJson.get("publicKeyDilithium", "").asString());
    block.publicKeyDilithium =
        std::vector<unsigned char>(decoded.begin(), decoded.end());
  }
  {
    std::string decoded =
        Crypto::base64Decode(blockJson.get("publicKeyFalcon", "").asString());
    block.publicKeyFalcon =
        std::vector<unsigned char>(decoded.begin(), decoded.end());
  }

  if (blockJson.isMember("accumulatedWork"))
    block.accumulatedWork =
        boost::multiprecision::cpp_int(blockJson["accumulatedWork"].asString());
...
alyncoin::BlockProto Block::toProtobuf() const {
  ...
  std::string blkSig =
      blockSignature.empty()
          ? Crypto::blake3(
                hash +
                std::string(dilithiumSignature.begin(),
                            dilithiumSignature.end()) +
                std::string(falconSignature.begin(), falconSignature.end()))
          : blockSignature;

  // --- Enforce hash normalisation (will abort if not) ---
  if (hash.size() != 64 || previousHash.size() != 64) {
    std::cerr << "❌ [toProtobuf] Hash width invariant violated! hash=" << hash
              << " prev=" << previousHash << std::endl;
    abort();
  }
  ...
  if (txRoot.empty() ||
    ...
    std::cerr << "\n\n❌ [toProtobuf] FATAL: Block being serialized without "
                 "valid Merkle root! index="
              << index << " hash=" << hash << "\n";
  ...
  if (txRoot.empty()) {
    ...
    std::cerr << "[toProtobuf] WARNING: getTransactionsHash() empty, using "
                 "EMPTY_TX_ROOT_HASH!\n";
  }
  ...
  // === Binary fields ===
  if (!zkProof.empty()) {
    proto.set_zk_stark_proof(reinterpret_cast<const char *>(zkProof.data()),
                             zkProof.size());
  }
  proto.set_dilithium_signature(
      reinterpret_cast<const char *>(dilithiumSignature.data()),
      dilithiumSignature.size());
  proto.set_falcon_signature(
      reinterpret_cast<const char *>(falconSignature.data()),
      falconSignature.size());

  if (!publicKeyDilithium.empty()) {
    if (publicKeyDilithium.size() != 1312) {
      std::cerr << "❌ [toProtobuf] Unexpected Dilithium public key size: "
                << publicKeyDilithium.size() << "\n";
    }
    proto.set_public_key_dilithium(
        reinterpret_cast<const char *>(publicKeyDilithium.data()),
        publicKeyDilithium.size());
  }
  if (!publicKeyFalcon.empty()) {
    if (publicKeyFalcon.size() != FALCON_PUBLIC_KEY_BYTES) {
      std::cerr << "❌ [toProtobuf] Unexpected Falcon public key size: "
                << publicKeyFalcon.size() << "\n";
    }
    proto.set_public_key_falcon(
        reinterpret_cast<const char *>(publicKeyFalcon.data()),
        publicKeyFalcon.size());
  }
  ...
  for (const auto &l2tx : l2Transactions) {
    if (l2tx.getSender().empty() || l2tx.getRecipient().empty() ||
        l2tx.getAmount() <= 0.0 || l2tx.getSignatureDilithium().empty() ||
        l2tx.getSignatureFalcon().empty() || l2tx.getZkProof().empty()) {
      continue;
    }
  ...
  if (!epochProof.empty())
    proto.set_epoch_proof(reinterpret_cast<const char *>(epochProof.data()),
                          epochProof.size());
...
Block Block::fromProto(const alyncoin::BlockProto &protoBlock,
  ...
  auto safeStr = [&](const std::string &val, const std::string &label,
                     size_t maxLen = 10000) -> std::string {
    if (val.empty()) {
      if (!allowPartial)
  ...
  auto safeBinaryField = [&](const std::string &bin, const std::string &label,
                             size_t maxLen =
                                 10000) -> std::vector<unsigned char> {
    if (bin.empty()) {
      if (!allowPartial)
        std::cerr << "❌ [fromProto] Required binary field " << label
                  << " is empty.\n";
      return {};
    }
    if (bin.size() > maxLen) {
      std::cerr << "❌ [fromProto] " << label << " too large: " << bin.size()
                << " bytes\n";
      if (!allowPartial)
    ...
    if (label == "Falcon Public Key" &&
        binary.size() != FALCON_PUBLIC_KEY_BYTES) {
      std::cerr << "❌ [fromProto] Invalid Falcon Public Key length: "
                << binary.size() << " (expected: " << FALCON_PUBLIC_KEY_BYTES
                << ")\n";
      if (!allowPartial)
        throw std::runtime_error("Invalid Falcon public key length.");
      return {};
    }
    if (label == "Dilithium Public Key" &&
        binary.size() != DILITHIUM_PUBLIC_KEY_BYTES) {
      std::cerr << "❌ [fromProto] Invalid Dilithium Public Key length: "
                << binary.size() << " (expected: " << DILITHIUM_PUBLIC_KEY_BYTES
                << ")\n";
      if (!allowPartial)
  ...
  auto optionalStr = [&](const std::string &s, const std::string &label,
                         size_t maxLen = 128) -> std::string {
    if (s.empty()) {
  ...
  auto optionalBinaryField =
      [&](const std::string &bin, const std::string &label,
          size_t maxLen = 10000) -> std::vector<uint8_t> {
    if (bin.empty()) {
      static bool warned = false;
      return {};
    }
  ...
  try {
    ...
    newBlock.previousHash = Crypto::normaliseHash(
        safeStr(protoBlock.previous_hash(), "previous_hash"));
    newBlock.hash = Crypto::normaliseHash(safeStr(protoBlock.hash(), "hash"));
    newBlock.minerAddress =
        safeStr(protoBlock.miner_address(), "miner_address");
    ...
    newBlock.blockSignature =
        safeStr(protoBlock.block_signature(), "block_signature");
    ...
    std::string protoTxRoot =
        safeStr(protoBlock.tx_merkle_root(), "tx_merkle_root", 128);
    ...
    newBlock.zkProof =
        safeBinaryField(protoBlock.zk_stark_proof(), "zkProof", 2'000'000);
    newBlock.dilithiumSignature = safeBinaryField(
        protoBlock.dilithium_signature(), "Dilithium Signature", 5000);
    newBlock.falconSignature = safeBinaryField(protoBlock.falcon_signature(),
                                               "Falcon Signature", 2000);
    newBlock.publicKeyDilithium = safeBinaryField(
        protoBlock.public_key_dilithium(), "Dilithium Public Key", 2000);
    newBlock.publicKeyFalcon = safeBinaryField(protoBlock.public_key_falcon(),
                                               "Falcon Public Key", 2000);

    // ---- ONLY soft for epoch fields! ----
    newBlock.epochRoot =
        optionalStr(protoBlock.epoch_root(), "epoch_root", 128);
    newBlock.epochProof =
        optionalBinaryField(protoBlock.epoch_proof(), "epoch_proof", 5000);

    if (!protoBlock.accumulated_work().empty())
      newBlock.accumulatedWork =
          boost::multiprecision::cpp_int(protoBlock.accumulated_work());
    else
  ...
  for (const auto &protoTx : protoBlock.transactions()) {
    try {
      ...
      if (!isReward &&
          (tx.getSender().empty() || tx.getRecipient().empty() ||
           tx.getAmount() <= 0.0 || tx.getSignatureDilithium().empty() ||
           tx.getSignatureFalcon().empty() || tx.getZkProof().empty())) {
        std::cerr
            << "⚠️ [fromProto] Skipping tx with missing or invalid fields.\n";
        skipped++;
        continue;
      }
  ...
  if (newBlock.transactions.empty() && protoBlock.transactions_size() > 0 &&
      !allowPartial) {
