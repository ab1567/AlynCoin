      ...
      //
        ...
        if (healer) {
          ...
          nlohmann::json statusJson = {{"healthy", status.isHealthy},
                                      ...
                                      {"height_gap", status.heightGap},
                                      {"remote_stronger", status.remoteStronger},
                                      {"tip_mismatch", status.tipMismatch},
                                      {"local_work", status.localWork},
                                      {"remote_work", status.remoteWork},
...
int main(int argc, char *argv[]) {
  ...
  if (network && !connectIP.empty() && getAppConfig().allow_manual_peers) {
    ...
    auto outcome =
        alyn::cli::connectPeerWithFeedback(*network, ip, connectPort,
